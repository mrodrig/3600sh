...
#define UNRECOGNIZED_SEQUENCE  1
...
status *generate_status() {
  ...
  if (s == NULL) {
    s->status_code = ALLOCATION_FAILURE; // failed to allocate the memory for our status struct
    return NULL;
  }
  ...
  s->parse_mode     = TRUE;      // default: s->parse_mode     = 1 -- are we currently parsing input from the user?
...
status *read_and_parse() {
  ...
  if (s->status_code == UNRECOGNIZED_SEQUENCE) {
    printf("Error: Unrecognized escape sequence.\n");
  } else if (s->status_code == INVALID_SYNTAX) {
    printf("Error: Invalid syntax.\n");
  } else if (s->status_code == ALLOCATION_FAILURE) {
    printf("Error: Failure during attempt to allocate status space.\n");
  }
  insert_arg(s->argv, NULL); // insert NULL at end so we know where the end is
  return s;
}
...
void parse_input(status *s, char c) {
  if (!s->parse_mode) { return ; } // if not parsing, do nothing
  if (!s->escape_mode) { // if we aren't in escape mode
    if (c == ' ' || c == '\t' || c == '\n' || c == EOF) { // end of line or arg delimiters
      if (s->redirect_type == NO_REDIR) { // if we haven't seen any redirects
        char *as = retrieve_arg_string(s->arg_string);
        if ((strcmp("<", as)) == 0) { // if current arg is the inp redir char
          ...
          if (arg == NULL) { // if the calloc fails,
            s->status_code = ALLOCATION_FAILURE; // set a failed alloc status code
            ...
            s->status_code = INVALID_SYNTAX; // illegal syntax
            s->parse_mode = FALSE; // stop parsing
          } else { // otherwise,
            sprintf(arg, "%s", as); // copy the arg into the mem location
        ...
        if ((strcmp("<", as) == 0) || // if the arg is an inp redir char
          ...
          s->status_code = ALLOCATION_FAILURE; // if the calloc fails, set a failed alloc status code
          s->parse_mode = FALSE; // stop parsing
        } else {
          sprintf(s->fds[s->redirect_type], "%s", as); // copy the file name into the fds array
        }
        s->redirect_type = NO_REDIR; // reset the redirect type
      } else if (c == '\n' || c == EOF) {
        s->status_code = INVALID_SYNTAX;
        s->parse_mode = FALSE;
      }
      ...
      s->status_code = INVALID_SYNTAX; // mark this as illegal syntax
      s->parse_mode = FALSE; // stop parsing
    } else if (c == '&') { // haven't seen a '&' yet,
      s->bg_proc = TRUE; // set this as a background process
    } else if (s->bg_proc) {
      s->status_code = INVALID_SYNTAX; // illegal syntax
      s->parse_mode = FALSE; // stop parsing
    ...
    if (c == ' ' || c == '&' || c == '\\') { // if we see these legal characters, we can append them exactly as is
      ...
      s->status_code = UNRECOGNIZED_SEQUENCE; // unrecognized escape sequence
      s->parse_mode = FALSE; // stop parsing
    }
