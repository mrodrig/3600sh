...
int execute_cmd(status *s) {
  ...
  if (pid == 0) { // child process code
    ...
    int error = 0; // do we have any errors opening redirection files?
    ...
    if (error > 0) { exit(1); } // if we hit an error redirecting io
    if (execvp(exec_argv[0], exec_argv) == -1) { // if there was an error with execvp
      // swap child's file descriptors back to parent's before printing errors
